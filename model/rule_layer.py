...
def rule_conv_block(X, nr, pretrain, skip=False, n_filter=16, s_filter=5, pool_by_stride=False, activation=binarize_STE,
...
def input_exp(X, nr, activation=binarize_STE, ind_scaling=False):
    with tf.variable_scope("input" + str(nr), reuse=False):
...
def rule_skipreduce_eff(X, nr):
    s_filter, n_filter = 1, 1
    with tf.variable_scope("umbrella_conv_" + str(nr), reuse=False):
        conv1 = tf.layers.conv2d(inputs=X, filters=n_filter, kernel_size=[s_filter, s_filter],
                                 padding="same", activation=None)
        thresholded = binarize(conv1)
        thresholded = tf.layers.batch_normalization(thresholded, training=True)
    return thresholded
