...
class network():
            ...
            if pretrain:
                for iteration in range(self.n_iterations):
                    ...
                    if iteration % self.check_every == 0:
                        indices = np.random.choice(len(test), 1000)
    ...
    def __init__(self, name, avg_pool, real_in, lr=1E-3, batch_size=2**8, activation=binarize_STE, pool_by_stride=False, bn_before=False, bn_after=False, ind_scaling=False, pool_before=False, pool_after=False, skip=False, pool_skip=False):
        ...
        self.folder_to_save, self.name = os.path.dirname(os.path.realpath(__file__)) + "/stored_models/" + str(name), name
        ...
        self.loss = tf.reduce_mean(-tf.reduce_sum(self.Y * tf.log(self.y + 1E-10), reduction_indices=[1]))  # + reg2
